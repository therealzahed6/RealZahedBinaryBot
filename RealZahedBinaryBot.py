import time, threading, traceback, requests, math, json, os, re, asyncio, asyncio
...
# ============================== QUOTEX API ==============================
# Install: pip install quotexpy
try:
    from quotexpy import Quotex
    HAS_QUOTEX = True
except ImportError:
    HAS_QUOTEX = False
    print("[WARNING] quotexapi not found. Run: pip install quotexpy")
    print("[WARNING] Auto-trading DISABLED — signals will show only.")
...
# ============================== QUOTEX CREDENTIALS ==============================
# Change these when you update your password via Pydroid
QUOTEX_EMAIL    = "zahedtrader2004@gmail.com"
QUOTEX_PASSWORD = "MoosaZahed06@"
QUOTEX_DEMO     = False   # Set True to use demo account, False for real account

# Global Quotex client + state
_qx_client       = None
_qx_connected    = False
_qx_lock         = threading.Lock()
_qx_trade_id     = None   # active trade ID
_qx_trade_amount = 1.0    # default, overwritten after user sets amount
...
async def _qx_connect_async():
    """Async connect to Quotex v1.40.x"""
    global _qx_client, _qx_connected
    try:
        _qx_client = Quotex(
            email=QUOTEX_EMAIL,
        ...
        status, reason = await _qx_client.connect()
        if status:
            _qx_client.change_balance("PRACTICE" if QUOTEX_DEMO else "REAL")
            _qx_connected = True
            return True, "Connected to Quotex ✅"
        else:
            _qx_connected = False
...
def qx_connect() -> tuple:
    """Connect to Quotex using quotexpy. Returns (success: bool, message: str)."""
    global _qx_client, _qx_connected
    if not HAS_QUOTEX:
        return False, "quotexpy library not installed. Run: pip install quotexpy"
    with _qx_lock:
        try:
            return _run_async(_qx_connect_async())
        except Exception as e:
            _qx_connected = False
            return False, f"Error: {str(e)}"

async def _qx_get_pairs_async(min_payout):
    """Async get payment pairs."""
    try:
        all_assets = await _qx_client.get_payment()
        selected = []
        if isinstance(all_assets, dict):
            for name, info in all_assets.items():
                is_otc = "OTC" in name.upper() or "_otc" in name.lower()
                if isinstance(info, dict):
                    payout = info.get("profit", info.get("payout", 0))
                    is_open = info.get("open", True)
                else:
                    payout = float(info) if info else 0
                    is_open = True
                if not is_otc and is_open and payout >= min_payout:
                    selected.append({"name": name, "payout": int(payout)})
        selected.sort(key=lambda x: x["payout"], reverse=True)
        return selected
    except Exception as e:
        print(f"[QUOTEX] get_live_pairs error: {e}")
        return []

def qx_get_live_pairs_high_payout(min_payout: int = 80) -> list:
    """Return live (non-OTC) pairs with payout >= min_payout%."""
    global _qx_client
    if not HAS_QUOTEX or not _qx_connected or _qx_client is None:
        return []
    try:
        return _run_async(_qx_get_pairs_async(min_payout))
    except Exception as e:
        print(f"[QUOTEX] get_live_pairs error: {e}")
        return []

async def _qx_buy_async(amount, asset, action, duration_seconds):
    """Async place trade."""
    status, trade_id = await _qx_client.buy(
        price=amount,
        asset=asset,
        direction=action,
...
def qx_place_trade(pair: str, direction: str, amount: float, duration_seconds: int) -> tuple:
    """
    Place a binary trade on Quotex using quotexpy.
    direction: 'CALL' or 'PUT'
    Returns (success: bool, trade_id or error_msg)
    """
    global _qx_client, _qx_connected, _qx_trade_id
    if not HAS_QUOTEX or not _qx_connected or _qx_client is None:
    ...
    try:
        with _qx_lock:
            status, trade_id = _run_async(_qx_buy_async(amount, asset, action, duration_seconds))
        if status:
            _qx_trade_id = trade_id
            return True, trade_id
        else:
            return False, "Trade rejected by Quotex"
    except Exception as e:
        return False, str(e)

async def _qx_check_win_async(trade_id):
    """Async check win."""
    return await _qx_client.check_win(trade_id)

def qx_check_result(trade_id) -> str:
    """
    Poll Quotex for trade result using quotexpy.
    Returns 'profit', 'loss', or 'refund'.
    """
    global _qx_client
    if not HAS_QUOTEX or _qx_client is None:
        return "unknown"
    try:
        result = _run_async(_qx_check_win_async(trade_id))
        if result is None:
            return "refund"
        ...
        print(f"[QUOTEX] check_result error: {e}")
        return "unknown"

async def _qx_balance_async():
    """Async get balance."""
    return await _qx_client.get_balance()

def qx_get_balance() -> Optional[float]:
    """Get current account balance using quotexpy."""
    global _qx_client
    if not HAS_QUOTEX or _qx_client is None:
        return None
    try:
        bal = _run_async(_qx_balance_async())
        return float(bal)
    except Exception:
        return None
...
STATE = {
    ...
    "awaiting_trade_amount": False,    # True when bot is waiting for trade amount input
    "trade_amount": 1.0,               # Fixed trade amount set by user
...
def cmd_start(msg):
    ...
    with state_lock:
        ...
        STATE["awaiting_capital"] = False
        STATE["awaiting_done_capital"] = False
        STATE["awaiting_trade_amount"] = False

    # ── Step 1: Send startup message ─────────────────────────────────────
    safe_send(chat_id, "RealZahedBinary Bot Started 🚀")

    # ── Step 2: Connect to Quotex in background ───────────────────────────
    def _connect_and_continue():
        if HAS_QUOTEX:
            safe_send(chat_id, "⏳ Connecting to Quotex...")
            ok, conn_msg = qx_connect()
            safe_send(chat_id, conn_msg)
            if not ok:
                safe_send(chat_id,
                    "⚠️ Could not connect to Quotex.\n"
                    "Signals will still show but trades won't be placed automatically.\n"
                    "Use /start to try again."
                )
        else:
            safe_send(chat_id,
                "⚠️ <b>quotexpy not installed</b>\n\n"
                "Run this on your server:\n"
                "<code>pip install quotexpy</code>\n\n"
                "Signals will still show but auto-trading is disabled."
            )

        # ── Step 3: Scan for high-payout live pairs ───────────────────────
        if HAS_QUOTEX and _qx_connected:
            safe_send(chat_id, "⏳ Scanning pairs for 80%+ payout...")
            live_pairs = qx_get_live_pairs_high_payout(80)
            if live_pairs:
                pairs_text = "\n".join([f"  • {p['name']} — {p['payout']}% payout" for p in live_pairs[:15]])
                safe_send(chat_id,
                    f"Pairs Selected ✅\n\n"
                    f"<b>Live Pairs (≥80% Payout):</b>\n{pairs_text}\n\n"
                    f"<i>Total: {len(live_pairs)} pairs available</i>"
                )
                # Map Quotex pair names back to our signal pair format
                quotex_pair_names = [p["name"] for p in live_pairs]
                # Filter our ALL_PAIRS list to only those available on Quotex with high payout
                matched = [p for p in ALL_PAIRS if p.replace("/","") in quotex_pair_names or p in quotex_pair_names]
                if not matched:
                    matched = ALL_PAIRS  # fallback: use all if mapping fails
                with state_lock:
                    STATE["_quotex_live_pairs"] = live_pairs
                    STATE["_quotex_matched_pairs"] = matched
            else:
                safe_send(chat_id,
                    "⚠️ Could not fetch live pairs from Quotex.\n"
                    "Using default pairs list instead."
                )

        # ── Step 4: Ask for trade amount ──────────────────────────────────
        with state_lock:
            current_amount = STATE.get("trade_amount", 1.0)
        safe_send(chat_id,
            f"💰 <b>Enter Trade Amount</b>\n\n"
            f"How much do you want to trade per signal?\n\n"
            f"Current: <b>${current_amount:.2f}</b>\n\n"
            f"Reply with the amount, e.g. <code>10</code> or <code>25</code>"
        )
        with state_lock:
            STATE["awaiting_trade_amount"] = True

    threading.Thread(target=_connect_and_continue, daemon=True).start()
...
def cb_signal_accept(cq):
    """Signal accepted — auto-place trade on Quotex and record it."""
    if not is_authorized(cq.message.chat.id):
        bot.answer_callback_query(cq.id, "❌ Unauthorized")
        return
    ...
    with state_lock:
        ...
        trade_amount = STATE.get("trade_amount", _qx_trade_amount)

    # ── Auto-place trade on Quotex ────────────────────────────────────────
    pair      = sig_data.get("pair", "")
    direction = sig_data.get("direction", "")
    expiry_min = sig_data.get("expiry_min", 1)
    duration_sec = expiry_min * 60
    ...
    if HAS_QUOTEX and _qx_connected:
        ok, trade_result = qx_place_trade(pair, direction, trade_amount, duration_sec)
        if ok:
            new_text = (
                original_text +
                f"\n\n✅ <b>Trade Placed on Quotex!</b>\n"
                f"💰 Amount: <b>${trade_amount:.2f}</b>\n"
                f"🆔 Trade ID: <code>{trade_result}</code>"
            )
            safe_edit_text(cq.message.chat.id, cq.message.message_id, new_text)
            bot.answer_callback_query(cq.id, f"✅ Trade Placed — ${trade_amount:.2f} {direction}")
            print(f"[QUOTEX] ✅ Trade placed — Signal #{signal_no} | {pair} {direction} ${trade_amount} | ID: {trade_result}")

            # Store trade_id so result-checker can look it up
            with state_lock:
                STATE["_last_trade_id"] = trade_result
        else:
            new_text = (
                original_text +
                f"\n\n⚠️ <b>Quotex trade failed: {trade_result}</b>\n"
                f"Signal recorded but trade was NOT placed."
            )
            safe_edit_text(cq.message.chat.id, cq.message.message_id, new_text)
            bot.answer_callback_query(cq.id, f"⚠️ Trade failed: {trade_result}")
            print(f"[QUOTEX] ❌ Trade failed — Signal #{signal_no} | {pair} | {trade_result}")
    else:
        # Not connected — just mark as accepted without trading
        new_text = original_text + "\n\n✅ <b>Signal Accepted</b> (Auto-trade unavailable — not connected to Quotex)"
        safe_edit_text(cq.message.chat.id, cq.message.message_id, new_text)
        bot.answer_callback_query(cq.id, f"✅ Signal #{signal_no:02d} Accepted (no auto-trade)")
        print(f"[SIGNAL] #{signal_no} accepted (Quotex not connected — no trade placed)")
...
def handle_all_messages(msg):
    ...
    with state_lock:
        ...
        awaiting_amount = STATE.get("awaiting_trade_amount", False)
        prev_capital_state = STATE.get("capital")

    # ── Handle trade amount reply (from /start flow) ───────────────────────
    if awaiting_amount:
        try:
            ...
            safe_send(msg.chat.id,
                "⚠️ Invalid amount. Please enter a number, e.g. <code>10</code> or <code>25</code>"
            )
            return
        with state_lock:
            STATE["awaiting_trade_amount"] = False
            STATE["trade_amount"] = amount
        global _qx_trade_amount
        _qx_trade_amount = amount
        safe_send(msg.chat.id,
            f"✅ <b>Trade Amount Set: ${amount:.2f}</b>\n\n"
            f"💰 Each signal will place a <b>${amount:.2f}</b> trade automatically.\n\n"
            f"📊 Now select your trading pairs:"
        )
        # Show pair selector
        kb = types.InlineKeyboardMarkup(row_width=2)
        for p in ALL_PAIRS:
        ...
        safe_send(
            msg.chat.id,
            "Tap pairs to select/deselect, then press <b>START BOT</b>",
            reply_markup=kb
        )
        return
...
def format_signal_message(signal: Dict, signal_no: int, advance_seconds: int = 0) -> str:
    ...
    return (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🤖 <i>Trade will be placed automatically at entry time</i>"
    )
...
def send_signal_with_buttons(chat_id: int, signal: Dict, signal_no: int, advance_seconds: int = 0) -> Optional[types.Message]:
    """Send signal message and AUTO-ACCEPT it (place trade automatically). No buttons needed."""
    with _sent_signals_lock:
    ...
    msg_text += (
        ...
        f"💰 <b>Trade Amount:</b> ${trade_amount:.2f}\n"
        f"🤖 <b>Auto-trading ON</b> — trade will be placed automatically at entry time."
    )

    # Send signal to Telegram (no buttons)
    result = safe_send(chat_id, msg_text)
    if result is None:
        with _sent_signals_lock:
            _sent_signal_nos.discard(signal_no)
        return None

    # ── Auto-accept and place trade immediately ────────────────────────────
    def _auto_accept():
        # Wait until entry time
        entry_time = signal.get("entry_time")
        if entry_time:
            wait_secs = max(0, (entry_time - now_utc()).total_seconds())
            if wait_secs > 0:
                print(f"[AUTO-TRADE] Waiting {wait_secs:.0f}s until entry time for Signal #{signal_no}...")
        ...
        user_record_signal(
            ...
            entry_ist=signal.get("entry_ist", ""),
            expiry_ist=signal.get("expiry_ist", ""),
            expiry_minutes=signal.get("expiry_min", 1),
        ...
        # Bump signal counter
        next_no = signal_no + 1
        user_save_signal_no(chat_id, next_no)
        save_signal_no_for_today(next_no)
        with state_lock:
            STATE["signal_no"] = next_no
            t_amount = STATE.get("trade_amount", _qx_trade_amount)

        # ── Place trade on Quotex ─────────────────────────────────────────
        if HAS_QUOTEX and _qx_connected:
            pair      = signal.get("pair", "")
            direction = signal.get("direction", "")
            expiry_min = signal.get("expiry_min", 1)
            ok, trade_result = qx_place_trade(pair, direction, t_amount, expiry_min * 60)
            if ok:
                with state_lock:
                    STATE["_last_trade_id"] = trade_result
                safe_send(chat_id,
                    f"✅ <b>Signal #{signal_no:02d} — Trade Placed!</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"💹 <b>Pair:</b> {pair}\n"
                    f"🎯 <b>Direction:</b> {'🟢 CALL ▲' if direction == 'CALL' else '🔴 PUT ▼'}\n"
                    f"💰 <b>Amount:</b> ${t_amount:.2f}\n"
                    f"⏱️ <b>Expiry:</b> {expiry_min} min\n"
                    f"🆔 <b>Trade ID:</b> <code>{trade_result}</code>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"⏳ Waiting for result..."
                )
                print(f"[AUTO-TRADE] ✅ Signal #{signal_no} | {pair} {direction} ${t_amount} | ID: {trade_result}")
            else:
                with state_lock:
                    STATE["_last_trade_id"] = None
                safe_send(chat_id,
                    f"⚠️ <b>Signal #{signal_no:02d} — Trade Failed!</b>\n"
                    f"💹 Pair: {pair}\n"
                    f"❌ Error: {trade_result}\n\n"
                    f"Signal is recorded. Please trade manually if needed."
                )
                print(f"[AUTO-TRADE] ❌ Signal #{signal_no} trade failed: {trade_result}")
        else:
            with state_lock:
                STATE["_last_trade_id"] = None
            safe_send(chat_id,
                f"⚠️ <b>Signal #{signal_no:02d}</b> — Auto-trade unavailable (not connected to Quotex).\n"
                f"Please trade manually."
            )
...
# ============================== AUTO RESULT DETECTION (QUOTEX) ==============================

def auto_detect_and_send_result(chat_id: int, signal_no: int, pair: str, expiry_ist: str, trade_id):
    """
    After trade expires: poll Quotex for result automatically.
    If Quotex returns result → send it to Telegram and record it.
    If not connected or fails → fall back to manual Profit/Loss buttons.
    """
    if not HAS_QUOTEX or not _qx_connected or trade_id is None:
        # Fallback to manual buttons
        ask_signal_result(chat_id, signal_no, pair, expiry_ist)
        return

    # Poll Quotex up to 10 times (30 seconds total)
    result = None
    for attempt in range(10):
        time.sleep(3)
        try:
            r = qx_check_result(trade_id)
            if r in ("profit", "loss", "refund"):
                result = r
                break
        except Exception as e:
            print(f"[QUOTEX] Result poll error attempt {attempt+1}: {e}")

    if result is None:
        # Could not get result — ask user manually
        print(f"[QUOTEX] Could not auto-detect result for trade {trade_id} — asking user manually")
        ask_signal_result(chat_id, signal_no, pair, expiry_ist)
        return

    # ── Got result automatically ──────────────────────────────────────────
    emoji_map = {"profit": "🟢", "loss": "🔴", "refund": "⚪"}
    label_map = {"profit": "WIN ✅", "loss": "LOSS ❌", "refund": "REFUND ↩️"}
    emoji = emoji_map.get(result, "❓")
    label = label_map.get(result, result.upper())

    with state_lock:
        trade_amount = STATE.get("trade_amount", _qx_trade_amount)

    if result == "profit":
        pnl = trade_amount * 0.80   # approx 80% payout
        pnl_text = f"+${pnl:.2f}"
    elif result == "loss":
        pnl_text = f"-${trade_amount:.2f}"
    else:
        pnl_text = "$0.00"

    balance = qx_get_balance()
    balance_line = f"\n💰 <b>Balance:</b> ${balance:.2f}" if balance is not None else ""

    result_msg = (
        f"{emoji} <b>SIGNAL #{signal_no:02d} — {label}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💹 <b>Pair:</b> {pair}\n"
        f"📊 <b>Result:</b> {label}\n"
        f"💵 <b>P&L:</b> <b>{pnl_text}</b>"
        f"{balance_line}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"<i>Result auto-detected from Quotex</i>"
    )
    safe_send(chat_id, result_msg)

    # Save result to memory
    update_signal_result(signal_no, result)
    user_update_signal_result(chat_id, signal_no, result)

    # Update auto-learning
    today = get_today_key()
    with memory_lock:
        mem = load_memory()
        sig_data_mem = mem.get("days", {}).get(today, {}).get("signals", {}).get(str(signal_no), {})
    entry_ist = sig_data_mem.get("entry_ist", "")
    if result in ("profit", "loss"):
        update_auto_learning(pair, result, entry_ist)
    cooldown_result = "win" if result == "profit" else ("loss" if result == "loss" else "pending")
    update_pair_cooldown_result(pair, cooldown_result)

    # Update session stats
    update_session_stats(result, trade_amount)

    # Update pair consecutive losses
    with state_lock:
        pcl = STATE.setdefault("pair_consecutive_losses", {})
        if result == "loss":
            pcl[pair] = pcl.get(pair, 0) + 1
        else:
            pcl[pair] = 0
    save_pair_losses(STATE["pair_consecutive_losses"])
    ...
    print(f"[QUOTEX] Auto-result for Signal #{signal_no}: {result} | {pnl_text}")
            ...
            if active:
                ...
                if expiry_time and now_utc() >= expiry_time:
                    ...
                    if recorded and recorded.get("accepted"):
                        # Set waiting_for_result flag — scheduler pauses until result received
                        with state_lock:
                            STATE["waiting_for_result"] = True
                            trade_id = STATE.get("_last_trade_id")

                        # Auto-detect result from Quotex (falls back to manual if not connected)
                        threading.Thread(
                            target=auto_detect_and_send_result,
                            args=(chat_id, sig_no, active["pair"], active["expiry_ist"], trade_id),
                            daemon=True
                        ).start()

                        print(f"[SCHEDULER] Auto-result detection started for Signal #{sig_no}...")
                        ...
                        if still_waiting:
                            ...
                            safe_send(
                                ...
                                f"Could not auto-detect result for <b>{active['pair']}</b>.\n\n"
