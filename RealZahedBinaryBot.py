...
def qx_connect() -> tuple:
    ...
    with _qx_lock:
        try:
            ...
            # quotexpy uses change_balance for account type
            status, reason = _qx_client.connect()
            if status:
                # Set account type: REAL or PRACTICE
                _qx_client.change_balance("REAL" if not QUOTEX_DEMO else "PRACTICE")
                _qx_connected = True
...
def qx_place_trade(pair: str, direction: str, amount: float, duration_seconds: int) -> tuple:
    ...
    try:
        with _qx_lock:
            status, trade_id = _qx_client.buy(
                price=amount,
                asset=asset,
                direction=action,
...
def qx_get_balance() -> Optional[float]:
    ...
    if not HAS_QUOTEX or _qx_client is None:
        return None
    try:
        return float(_qx_client.get_balance())
    except Exception:
        return None
