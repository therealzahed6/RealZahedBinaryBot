...
def qx_connect() -> tuple:
    ...
    with _qx_lock:
        try:
            ...
            result = _qx_client.connect()
            if asyncio.iscoroutine(result):
                result = _run_async(result)
            if isinstance(result, tuple):
                status, reason = result
            else:
                status, reason = bool(result), "unknown"
            if status:
                cb = _qx_client.change_balance("REAL" if not QUOTEX_DEMO else "PRACTICE")
...
def qx_get_live_pairs_high_payout(min_payout: int = 80) -> list:
    ...
    if not HAS_QUOTEX or not _qx_connected or _qx_client is None:
        return []
    try:
        # quotexpy: get_payment() returns dict of {asset: payout_percent}
        all_assets = _qx_client.get_payment()
        if asyncio.iscoroutine(all_assets):
            all_assets = _run_async(all_assets)
        selected = []
        if isinstance(all_assets, dict):
            for name, info in all_assets.items():
                is_otc = "OTC" in name.upper() or "_otc" in name.lower()
                # info can be a number (payout %) or dict
...
def qx_place_trade(pair: str, direction: str, amount: float, duration_seconds: int) -> tuple:
    ...
    # quotexpy uses "call" / "put" lowercase
    action = "call" if direction.upper() == "CALL" else "put"
    # Convert pair format: "EUR/USD" → "EURUSD"
    asset = pair.replace("/", "")
    try:
        with _qx_lock:
            buy_result = _qx_client.buy(
                price=amount,
                asset=asset,
                direction=action,
                duration=duration_seconds,
            )
            if asyncio.iscoroutine(buy_result):
                buy_result = _run_async(buy_result)
            if isinstance(buy_result, tuple):
                status, trade_id = buy_result
            else:
                status, trade_id = bool(buy_result), None
        if status:
...
def qx_check_result(trade_id) -> str:
    ...
    if not HAS_QUOTEX or _qx_client is None:
        return "unknown"
    try:
        result = _qx_client.check_win(trade_id)
        if asyncio.iscoroutine(result):
            result = _run_async(result)
        # quotexpy check_win: returns win amount (>0 = profit), 0 = loss, negative = loss
        if result is None:
            return "refund"
