...
# ============================== QUOTEX API ==============================
# Install: pip install quotexapi
try:
    from quotexapi.stable_api import Quotex
    HAS_QUOTEX = True
except ImportError:
    HAS_QUOTEX = False
    print("[WARNING] quotexapi not found. Run: pip install quotexapi")
...
def qx_connect() -> tuple:
    ...
    if not HAS_QUOTEX:
        return False, "quotexapi library not installed. Run: pip install quotexapi"
    with _qx_lock:
        try:
            _qx_client = Quotex(
                email=QUOTEX_EMAIL,
            ...
            _qx_client.set_account_mode("REAL" if not QUOTEX_DEMO else "PRACTICE")
...
def qx_get_live_pairs_high_payout(min_payout: int = 80) -> list:
    ...
    if not HAS_QUOTEX or not _qx_connected or _qx_client is None:
        return []
    try:
        assets = _qx_client.get_all_asset()
        selected = []
        for asset in assets:
            name    = asset.get("name", "")
            payout  = asset.get("payout", 0)
            is_open = asset.get("open", False)
            is_otc  = "OTC" in name.upper()
            if not is_otc and is_open and payout >= min_payout:
                selected.append({"name": name, "payout": payout})
...
def qx_place_trade(pair: str, direction: str, amount: float, duration_seconds: int) -> tuple:
    """
    Place a binary trade on Quotex.
    ...
    # Quotex uses 'call' / 'put' lowercase
...
def qx_check_result(trade_id) -> str:
    ...
    try:
        ...
        # quotexapi returns: True = win, False = loss, None = refund/tie
        if result is True:
            return "profit"
...
def cmd_start(msg):
    ...
    def _connect_and_continue():
        if HAS_QUOTEX:
            ...
            safe_send(chat_id,
                "⚠️ <b>quotexapi not installed</b>\n\n"
                "Run this on your server:\n"
                "<code>pip install quotexapi</code>\n\n"
